...
class measure extends Component {
  ...
  delete(id) {
    ...
    if (answer) {
      axios.post('http://localhost:3001/product/measure/delete',
        {
          id: id,
        }).then((respone) => {
  ...
  render() {
    ...
    const data = this.state.dataKu.map((item, index) => {
      ...
      return <tr key={index}>
          ...
          <th>{name_measure}</th>
          <th>
        <Link  to={{pathname: '/editmeasure', state: {id:id}}} className="btn btn-warning fa fa-pencil"></Link> &nbsp;
      ...
      <div>
        ...
        <main className="app-content">
          <div className="app-title">
            <div>
              <h1><i className="fa fa-cube"></i> Unit of Measure</h1><br />
              <Link className="btn btn-secondary btn-sm fa fa-arrow-left" to="/productall"> Back </Link> &nbsp;&nbsp;
              <Link className="btn btn-success btn-sm fa fa-plus" to="/addmeasure"> Add Measure </Link>
            </div>
            <ul className="app-breadcrumb breadcrumb side">
              <li className="breadcrumb-item"><i className="fa fa-home fa-lg"></i></li>
              <li className="breadcrumb-item">Home</li>
              <li className="breadcrumb-item"><Link to="/productall">Product</Link></li>
              <li className="breadcrumb-item active">Unit of Measure</li>
            </ul>
          </div>
          ...
          <div className="col-md-12">
            <div className="tile">
              <h3 className="tile-title">Unit of Measure</h3>
